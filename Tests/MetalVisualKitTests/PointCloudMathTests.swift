//
//  PointCloudMathTests.swift
//  MetalVisualKitTests
//
//  Pure point-cloud projection, orientation and interaction maths.
//
...
final class PointCloudMathTests: XCTestCase {
    ...
    private let orientations: [UIInterfaceOrientation] = [
        .portrait, .landscapeLeft, .landscapeRight, .portraitUpsideDown
    ]

    func testPerspectiveMapsNearPlaneToZeroDepth() {
        let projection = PointCloudRenderer.perspective(fovY: .pi / 3, aspect: 1, near: 0.1, far: 100)

        let point = projection * SIMD4<Float>(0, 0, -0.1, 1)

        XCTAssertEqual(point.z / point.w, 0, accuracy: 1e-4)
    }

    func testPerspectiveMapsFarPlaneToOneDepth() {
        let projection = PointCloudRenderer.perspective(fovY: .pi / 3, aspect: 1, near: 0.1, far: 100)

        let point = projection * SIMD4<Float>(0, 0, -100, 1)

        XCTAssertEqual(point.z / point.w, 1, accuracy: 1e-3)
    }
    ...
    func testRotateToARCameraIsOrthonormal() {
        for orientation in orientations {
            ...
            XCTAssertEqual(
                abs(simd_determinant(upper)), 1, accuracy: 1e-5, "Orientation \(orientation.rawValue) is not rigid.")
        }
    }

    func testLocalToWorldCancelsOrientedViewMatrix() {
        let viewMatrix = PointCloudRenderer.lookAt(
            eye: SIMD3<Float>(1.4, 0.7, 2.8), center: SIMD3<Float>(0.2, -0.1, 0), up: SIMD3<Float>(0, 1, 0))

        for orientation in orientations {
            let localToWorld = PointCloudRenderer.localToWorld(viewMatrix: viewMatrix, orientation: orientation)

            let composed = viewMatrix * localToWorld

            let expected = PointCloudRenderer.rotateToARCamera(for: orientation)

            for column in 0..<4 { XCTAssertEqual(simd_length(composed[column] - expected[column]), 0, accuracy: 1e-5) }
        }
    }

    func testLookAtPlacesEyeAtOrigin() {
        let view = PointCloudRenderer.lookAt(eye: SIMD3<Float>(0, 0, 3), center: .zero, up: SIMD3<Float>(0, 1, 0))

        let transformed = view * SIMD4<Float>(0, 0, 3, 1)

        let position = SIMD3<Float>(transformed.x, transformed.y, transformed.z)

        XCTAssertEqual(simd_length(position), 0, accuracy: 1e-5)
    }

    func testDemoCameraDistanceFitsNarrowerFieldOfView() {
        ...
        let portraitDistance = PointCloudRenderer.demoCameraDistance(
            aspect: 0.46, fovY: fovY, sphereRadius: radius, margin: margin)

        let landscapeDistance = PointCloudRenderer.demoCameraDistance(
            aspect: 2.17, fovY: fovY, sphereRadius: radius, margin: margin)

        let portraitHalfX = atan(tan(fovY * 0.5) * 0.46)

        XCTAssertGreaterThan(portraitDistance, landscapeDistance)

        XCTAssertGreaterThanOrEqual(portraitDistance * sin(portraitHalfX), radius * margin - 1e-5)
    }

    func testPointSizeConvertsLayoutPointsToDrawablePixels() {
        XCTAssertEqual(
            PointCloudRenderer.drawablePointSize(
                8, drawableSize: CGSize(width: 1_206, height: 2_622), viewportPointSize: CGSize(width: 402, height: 874)
            ), 24, accuracy: 1e-5)

        XCTAssertEqual(PointCloudRenderer.drawablePointSize(8, drawableSize: .zero, viewportPointSize: .zero), 8)
    }

    func testLivePointSizeMatchesThreeTimesDisplayScale() {
        XCTAssertEqual(PointCloudRenderer.defaultLivePointSize, 3)

        XCTAssertEqual(
            PointCloudRenderer.drawablePointSize(
                PointCloudRenderer.defaultLivePointSize, drawableSize: CGSize(width: 1_206, height: 2_622),
                viewportPointSize: CGSize(width: 402, height: 874)), 9, accuracy: 1e-5)
    }

    func testConfidenceOpacityPreservesMediumConfidenceCue() {
    ...
    func testDemoOrbitRespondsToDragAndClampsElevation() {
        ...
        let moved = PointCloudRenderer.updatedDemoOrbit(initial, translation: SIMD2<Float>(100, -10_000))
    ...
    func testIdleCloudStartsAtComfortableViewingDistance() { XCTAssertEqual(PointCloudRenderer.defaultDemoZoom, 1.4) }
}
