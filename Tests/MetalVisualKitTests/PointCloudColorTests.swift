...
//  MetalVisualKitTests
//
//  Camera-colour, confidence and AR-session presentation-state tests.
//
...
final class PointCloudColorTests: XCTestCase {
    ...
    func testNonPlanarDepthBufferReportsFullSize() throws {
        let buffer = try makePixelBuffer(width: 256, height: 192, format: kCVPixelFormatType_DepthFloat32)
    ...
    func testCameraBufferReportsPerPlaneSizes() throws {
        let buffer = try makePixelBuffer(width: 1_920, height: 1_440, format: ARFrameTextures.cameraPixelFormat)
        ...
        XCTAssertEqual(luma.width, 1_920)

        XCTAssertEqual(luma.height, 1_440)
    ...
    func testColorModeShaderValuesAreConsecutiveWholeNumbers() {
        ...
        for value in values { XCTAssertEqual(value, value.rounded(), "colorMode \(value) must be a whole number.") }
    }
    ...
    func testNormalTrackingShowsNothing() { XCTAssertNil(PointCloudSessionMonitor.status(for: .normal).message) }

    func testPreparingStatesAreDistinctFromActionableLimits() {
        XCTAssertTrue(PointCloudSessionMonitor.Status.starting.isPreparing)

        XCTAssertTrue(PointCloudSessionMonitor.status(for: .limited(.initializing)).isPreparing)

        XCTAssertTrue(PointCloudSessionMonitor.status(for: .limited(.relocalizing)).isPreparing)

        XCTAssertFalse(PointCloudSessionMonitor.status(for: .limited(.excessiveMotion)).isPreparing)
    }

    func testPublicPhaseDistinguishesIntentFromCaptureState() {
        XCTAssertEqual(LiDARPointCloudView.phase(isActive: false, fallbackReason: nil, status: .tracking), .idle)

        XCTAssertEqual(
            LiDARPointCloudView.phase(isActive: false, fallbackReason: nil, status: .preparing("Starting…")), .idle)

        XCTAssertEqual(
            LiDARPointCloudView.phase(isActive: true, fallbackReason: "No LiDAR scanner.", status: .tracking),
            .fallback("No LiDAR scanner."))

        XCTAssertEqual(
            LiDARPointCloudView.phase(isActive: true, fallbackReason: nil, status: .preparing("Starting…")),
            .preparing("Starting…"))

        XCTAssertEqual(LiDARPointCloudView.phase(isActive: true, fallbackReason: nil, status: .tracking), .tracking)
    }
    ...
    func testUnauthorizedCameraIsReportedAsPermissionProblem() {
        let error = NSError(domain: ARErrorDomain, code: ARError.Code.cameraUnauthorized.rawValue)
    ...
    func testNonARErrorFallsBackToGenericMessage() {
        let error = NSError(domain: "test", code: 1)

        XCTAssertFalse(PointCloudSessionMonitor.message(for: error).isEmpty)
    }

    func testInterruptedStatusExplainsItself() { XCTAssertNotNil(PointCloudSessionMonitor.Status.interrupted.message) }

    // MARK: - MainActor Session State

    @MainActor func testSessionStartRepublishesPreparingState() {
