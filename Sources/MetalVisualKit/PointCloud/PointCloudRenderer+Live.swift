...
extension PointCloudRenderer {
    func prepareSignpostedLiveFrame() -> PointCloudLiveFrame? {
        let signpost = MetalVisualLog.signposter.beginInterval(
            "AR frame preparation"
        )

        defer {
            MetalVisualLog.signposter.endInterval(
                "AR frame preparation",
                signpost
            )
        }
    ...
    private func prepareLiveFrame() -> PointCloudLiveFrame? {
        ...
        guard frame.timestamp != lastRenderedFrameTimestamp else {
            return nil
        }

        guard let depthBinding = textures.depth(from: depthData) else {
            return nil
        }
        ...
        let cameraBinding = makeCameraBinding(
            frame: frame,
            textures: textures
        )
        ...
        if let retainedTexture = confidence.retainedTexture {
            retainedTextures.append(retainedTexture)
        }

        retainedTextures.append(
            contentsOf: cameraBinding.retainedTextures
        )

        let uniforms = makeLiveUniforms(
            camera: frame.camera,
            depthTexture: depthBinding.texture,
            colorMode: cameraBinding.colorMode,
        ...
        return PointCloudLiveFrame(
            ...
            cameraLumaTexture: cameraBinding.image?.luma.texture,
    ...
    private func makeCameraBinding(
        frame: ARFrame,
        textures: ARFrameTextures
    ) -> PointCloudCameraBinding {
        guard colorMode == .camera else {
            return PointCloudCameraBinding(
                image: nil,
                colorMode: colorMode,
                retainedTextures: []
            )
        ...
        return PointCloudCameraBinding(
            image: image,
            colorMode: .camera,
            retainedTextures: image.retainedTextures
        )
    }
    ...
    private var projectionViewportSize: CGSize {
    ...
    func encodeLive(
        ...
        frame: PointCloudLiveFrame
    ) {
        let signpost = MetalVisualLog.signposter.beginInterval(
            "Point cloud render encode"
        )
