...
final class ARFrameTextures {
    ...
    func confidence(from depthData: ARDepthData, matching depthTexture: MTLTexture) -> ConfidenceBinding? {
        guard let confidenceMap = depthData.confidenceMap else {
            guard let fallback = fallbackConfidenceTexture(width: depthTexture.width, height: depthTexture.height)
            else { return nil }
        ...
        guard binding.texture.width == depthTexture.width, binding.texture.height == depthTexture.height else {
    ...
    private func makeBinding(_ pixelBuffer: CVPixelBuffer, format: MTLPixelFormat, planeIndex: Int) -> TextureBinding? {
        ...
        let status = CVMetalTextureCacheCreateTextureFromImage(
            nil, cache, pixelBuffer, nil, format, size.width, size.height, planeIndex, &cvTexture)
        guard status == kCVReturnSuccess, let cvTexture, let texture = CVMetalTextureGetTexture(cvTexture) else {
            MetalVisualLog.renderer.error("Texture creation failed for plane \(planeIndex) (CVReturn \(status)).")
            return nil
        }
    ...
    func metalFormat(for pixelBuffer: CVPixelBuffer) -> MTLPixelFormat? {
        ...
        case kCVPixelFormatType_DepthFloat32, kCVPixelFormatType_OneComponent32Float: return .r32Float
        case kCVPixelFormatType_OneComponent8: return .r8Uint
        default: return nil
        }
    }
    ...
    private func fallbackConfidenceTexture(width: Int, height: Int) -> MTLTexture? {
        if let cached = cachedFallbackConfidence, cached.width == width, cached.height == height { return cached }
        ...
        bytes.withUnsafeBytes { buffer in
            ...
            texture.replace(
                region: MTLRegionMake2D(0, 0, width, height), mipmapLevel: 0, withBytes: base, bytesPerRow: width)
        }
